  ...
  describe "proto3" do
    it "decodes" do
      File.open("#{__DIR__}/../fixtures/test.data.proto3-encoded") do |io|
        ...
        # TODO: Encoding & decoding repeated proto3 enums is currently broken
        # test.enum.should eq [TestMessagesV3::SomeEnum::YES, TestMessagesV3::SomeEnum::NO, TestMessagesV3::SomeEnum::NEVER]
    ...
    it "encodes" do
      File.open("#{__DIR__}/../fixtures/test.data.proto3-encoded") do |io|
        test = TestMessagesV3::Test3.from_protobuf(io)
        encoded = test.to_protobuf
        test2 = TestMessagesV3::Test3.from_protobuf(encoded.rewind)
